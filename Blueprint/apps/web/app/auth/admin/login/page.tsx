...
export default function AdminLoginPage() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (r.user.accountType !== "ADMIN") throw new Error("Please use Employee login.");
  ...
  return (
    <div style={card}>
      ...
      <p style={sub}>Admins can create accounts and view all employees under their company.</p>

      <form onSubmit={onSubmit} style={{ display: "grid", gap: 12 }}>
        <label style={labelWrap}>
          <div style={label}>Email</div>
          <input value={email} onChange={(e) => setEmail(e.target.value)} required type="email" style={input} placeholder="admin@company.com" />
        </label>
        <label style={labelWrap}>
        ...
        <div style={{ fontSize: 13, color: "#475569" }}>
          New admin?{" "}
          <Link href="/auth/admin/register" style={link}>
            Create account
          </Link>
        </div>
      </form>
